...
# Label according to UCL dataset
colnames(data) <- c(
  "age",
  "sex",# 0 = female, 1 = male
  "cp", # chest pain 
...
# it also shows us that there are some missing values. There are "?"s in the dataset.
...
# we have since converted to NAs) R thinks that the levels for the factor are strings, but
# we know they are integers, so we'll first convert the strings to integiers...
...
# "?"s with NAs because "?" no longer appears in the list of factors for "ca" and "thal"

# Now we are ready to build a random forest.
set.seed(42)

# NOTE: For most machine learning methods, you need to divide the data manually into a "training" set and a "test" set. This allows you to train 
# the method using the training data, and then test it on data it was not originally trained on. 
# In contrast, Random Forests split the data into "training" and "test" sets for you. This is because Random Forests use bootstrapped
# data, and thus, not every sample is used to build every tree. The "training" dataset is the bootstrapped data and the "test" dataset is
# the remaining samples. The remaining samples are called the "Out-Of-Bag" (OOB) data.
...
# you can change the number of trees it makes (the default is 300) and the number of variables that it will consider at each step.

# Now we are ready to build a random forest.

# NOTE: If the thing we're trying to predict (in this case it is whether or not someone has heart disease) is a continuous number 
# (i.e. "weight" or "height"), then by default, randomForest() will set "mtry", the number of variables to consider at each step, 
# to the total number of variables divided by 3 (rounded down), or to 1 (if the division results in a value less than 1).
# If the thing we're trying to predict is a "factor" (i.e. either "yes/no" or "ranked"), then randomForest() will set mtry to 
# the square root of the number of variables (rounded down to the next integer value).

# In this example, "hd", the thing we are trying to predict, is a factor and there are 13 variables. 
# So by default, randomForest() will setmtry = sqrt(13) = 3.6 rounded down = 3
...
# Up to a point, the more trees in the forest, the better. You can tell when you've made enough when the OOB no longer improves.
...
# NOTE: After building a random forest with 500 tress, the graph does not make it clear that the OOB-error has settled on a value or,
# if we added more  trees, it would continue to decrease.So we do the whole thing again, but this time add more trees.
